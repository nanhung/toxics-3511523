...
# Description of the parameters used in the PBPK model for CPM for PND90 rats.

params <- c(
  BODYWT =0.3517,			#body weight (kg)
  MW = 391.3,          		#molecular weight for CPM  (g/mole)
  ...
  VOLGIC = 0.0437, 		#fraction of bodyweight to to GI
  ...
  FRLIVC = 0.183, 		#fraction of total plasma flow to liver (portal + hepatic arterial)
  ...
  VCYPC = 6515.7,     		#Vmax for liver CYP  (umol/hr/kg) 
  VCAEMC=1386.49,      		#Vmax for liver microsomal CES (umol/hr/kg)
  VCAECC=246.91,   		#Vmax for liver cytosolic CES (umol/hr/kg)
  VPLSC=802.3,    		#Vmax for plasma CES (umol/hr/kg)
  KMCYP=0.77,      		#Km for liver CYP (umol/l)
  KMCAEM=8.19,   			#Km for liver microsomal CES (umol/l)
  KMCAEC=0.78,    		#Km for liver cytosolic CES (umol/l)
  KMPLS=1.37   			#Km for plasma CES (umol/l)
)   
